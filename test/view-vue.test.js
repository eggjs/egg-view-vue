    ...
    it('should GET /render', () => {
      return request(app.callback())
        ...
        .expect(res => {
          ...
          assert(res.text.indexOf('egg-view-vue#vue server side render!') > -1);
        });
    });

    it('should GET /render23', () => {
      return request(app.callback())
        .get('/app')
        .expect(200)
        .expect(res => {
          assert(res.text.indexOf('data-server-rendered="true"') > -1);
          assert(res.text.indexOf('</body></html>') > -1);
          assert(res.text.indexOf('.page-container') > -1);
          assert(res.text.indexOf('window.__INITIAL_STATE__') > -1);
          assert(res.text.indexOf('/public/static/js/vendor.js"') > -1);
          assert(res.text.indexOf('/public/static/js/app/app.js"') > -1);
        });
    });
